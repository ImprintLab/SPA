...
# def selected_rater_from_df(args, multi_rater, names, pt_label_df_path, selected_rater_df_path, epoch): 

#     epoch = 18

#     pt_label_df = pd.read_csv(pt_label_df_path, dtype={'name': 'object'})
#     pt_label_df = pt_label_df[pt_label_df['epoch'] == int(epoch)]

#     selected_rater_df = pd.read_csv(selected_rater_df_path, dtype={'name': 'object'})
#     selected_rater_df = selected_rater_df[selected_rater_df['epoch'] == int(epoch)]

#     labels_torch_list = []
#     coords_torch_list = [] 
#     selected_rater_list = []
#     masks_torch_list = []
#     masks_ori_torch_list = []

#     for name_index, name in enumerate(names):

#         # previous label
#         temp_pt_label_df = pt_label_df[pt_label_df['name'] == str(name)]
#         label = temp_pt_label_df['label'].values
#         pt = temp_pt_label_df[['x', 'y']].values

#         labels_torch_list.append(torch.tensor(label, device=multi_rater.device))
#         coords_torch_list.append(torch.tensor(pt, device = multi_rater.device))

#         # mask
#         temp_selected_rater_df = selected_rater_df[selected_rater_df['name'] == str(name)]
#         selected_rater = temp_selected_rater_df.iloc[0, :-2].values.astype(int) # only one row
#         selected_rater = torch.tensor(selected_rater, device = multi_rater.device) 
#         selected_rater_list.append(selected_rater)
#         selected_rater = torch.nonzero(selected_rater == 1).squeeze()

#         if selected_rater.ndim == 0:
#             selected_rater = torch.tensor([selected_rater])

#         # mask
#         selected_rater_mask_ori = multi_rater[name_index, selected_rater, :, :, :]
#         selected_rater_mask = F.interpolate(selected_rater_mask_ori, size=(args.out_size, args.out_size), mode='bilinear', align_corners=False).mean(dim=0) 
#         selected_rater_mask = (selected_rater_mask > 0.5).float() # torch.Size([1, mask_size, mask_size]) #(selected_rater_mask >= 0.5).float()
#         masks_torch_list.append(selected_rater_mask)

#         selected_rater_mask_ori = selected_rater_mask_ori.mean(dim=0) 
#         selected_rater_mask_ori = (selected_rater_mask_ori > 0.5).float() #(selected_rater_mask_ori >= 0.5).float()
#         masks_ori_torch_list.append(selected_rater_mask_ori)


#     labels = torch.stack(labels_torch_list, dim=0)
#     coords = torch.stack(coords_torch_list, dim=0)
#     selected_rater = torch.stack(selected_rater_list, dim=0)
#     masks = torch.stack(masks_torch_list, dim=0)
#     masks_ori = torch.stack(masks_ori_torch_list, dim=0)

#     return labels, coords, selected_rater, masks, masks_ori



def selected_rater_from_df(args, multi_rater, names, selected_rater_df_path, epoch): 
    ...
    return selected_rater, masks, masks_ori