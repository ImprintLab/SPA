...
class ImageEncoderViT(nn.Module):
    def __init__(
        ...
        self.fcomb_latent_dim = 64
        self.fcomb = Fcomb(output_channels=out_chans, latent_dim=self.fcomb_latent_dim, no_convs_fcomb=4,  #latent_dim same as sample_generator
                           initializers={'w':'orthogonal', 'b':'normal'}, use_tile=True)

    def forward(self, x: torch.Tensor, weights, means, variances,
                num_sample=16) -> torch.Tensor:
        ...
        if self.pos_embed is not None:
            ...
            feature = feature + new_abs_pos

        for blk in self.blocks:
            feature = blk(feature)
            
        feature = self.neck(feature.permute(0, 3, 1, 2))
        ...
        for _ in range(num_sample):
            ...
            normal_samples_expanded = normal_samples.unsqueeze(2).expand(feature.size(0), self.fcomb_latent_dim, means.size(0))
            gaussian = normal_samples_expanded * torch.sqrt(variances_expanded) + means_expanded
            weights_expanded = weights.unsqueeze(0).unsqueeze(0).expand(feature.size(0), self.fcomb_latent_dim, means.size(0))
            mixture_gaussian = gaussian * weights_expanded
            sample_from_latent_space = torch.sum(mixture_gaussian, dim=2)

            feature_temp = self.fcomb.forward(feature, sample_from_latent_space.to(dtype=torch.float))
