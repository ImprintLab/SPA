...
class Fcomb(nn.Module):
    ...
    """
    def __init__(self, output_channels, latent_dim, no_convs_fcomb, initializers, use_tile=True):
...
class ProbabilisticUnet(nn.Module):
    ...
    output_channels: the number of output channels from original features (out_chans = 256 in image encoder) 
    num_filters: is a list consisint of the amount of filters layer
    latent_dim: dimension of the latent space
    no_cons_per_block: no convs per block in the (convolutional) encoder of prior and posterior
    """

    def __init__(self, input_channels=3, output_channels=256, num_filters=[32,64,128,192], latent_dim=6, no_convs_per_block=3, no_convs_fcomb=4, beta=10.0):
        ...
        self.fcomb = Fcomb(self.output_channels, self.latent_dim, self.no_convs_fcomb, {'w':'orthogonal', 'b':'normal'}, use_tile=True).to(device)

    def forward(self, patch, original_feature, segm=None):
        """
        Construct prior latent space for patch and run patch through UNet,
        in case segm is not None (training is True) also construct posterior latent space
        """
        if segm is not None:
            self.posterior_latent_space = self.posterior.forward(patch, segm)
        self.prior_latent_space = self.prior.forward(patch)
        self.original_feature = original_feature  #self.unet.forward(patch,False)
    ...
    def sample(self, testing=False):
        ...
        if testing == False:
            ...
            self.z_prior_sample = z_prior
        return self.fcomb.forward(self.original_feature,z_prior)
    ...
    def reconstruct(self, use_posterior_mean=False, calculate_posterior=False, z_posterior=None):
        ...
        return self.fcomb.forward(self.original_feature, z_posterior)
